...
function add_whatsapp_chat() {
    ...
    ?>
    <a href="https://wa.me/<?php echo esc_attr( $whatsapp_number ); ?>?text=<?php echo urlencode( $default_message ); ?>" target="_blank" class="whatsapp-button" title="WhatsApp Chat">
        ...
        <form method="post">
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td><textarea id="default_message" name="default_message" class="large-text"><?php echo esc_textarea( $default_message ); ?></textarea></td>
                </tr>
                <tr>
                    <th scope="row"><label for="whatsapp_icon">WhatsApp Icon:</label></th>
                    <td>
                        <input type="text" id="whatsapp_icon" name="whatsapp_icon" value="<?php echo esc_url( $whatsapp_icon ); ?>" class="regular-text">
                        <button type="button" class="button button-secondary" id="whatsapp_icon_button">Select Icon</button>
                        <div><img src="<?php echo esc_url( $whatsapp_icon ); ?>" id="whatsapp_icon_preview" style="max-width: 60px; margin-top: 10px;"></div>
                    </td>
                </tr>
            </table>
            <p class="submit">
...
function whatsapp_chat_check_for_updates() {
    ...
    if ( version_compare( $latest_version, $current_version, '>' ) ) {
        add_action( 'admin_notices', 'whatsapp_chat_display_update_notification' );
    }
}
...
function whatsapp_chat_get_latest_version_from_github() {
    $url = 'https://api.github.com/repos/REstolDev/whatsapp-chat/tags';
...
// Muestra un aviso de actualización
function whatsapp_chat_display_update_notification() {
    ?>
    <div class="notice notice-info">
        <p>¡Una nueva versión del plugin WhatsApp Chat está disponible! Por favor, actualiza ahora para disfrutar de las últimas características y mejoras.</p>
    </div>
    <?php
}
